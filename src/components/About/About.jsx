...
//           transition={{ duration: 1 }}
//           viewport={{ once: false }}
//           className="w-[80%] max-w-md rounded-lg object-cover hover:scale-105 transition duration-300 shadow-lg"
//         />
...
const About = () => {
  return (
    <section
      id="about"
      className="flex flex-col items-center gap-20 mx-6 md:mx-20 lg:mx-40 my-20"
    >
      {/* Title */}
      <div className="relative text-center md:text-left">
        <h1 className="text-[40px] md:text-[60px] lg:text-[80px] font-bold px-2 md:px-8">
          About Me
        </h1>
        <img
          src={theme_pattern}
          alt="theme pattern"
          className="absolute bottom-0 right-0 -z-10 w-28 md:w-40"
        />
      </div>
          ...
          >
            {skills.map((item, index) => (
              <motion.div
                ...
                transition={{ duration: 0.6 }}
              >
                <p className="w-[150px] shrink-0 text-base md:text-lg font-medium m-0">
                  {item.skill}
                </p>
                <hr
                  className={`h-2 rounded-full border-none bg-gradient-to-r from-[#DF8908] to-[#B415FF] ${item.width}`}
      ...
      >
        ...
        <a
          href="https://leetcode.com/u/MuskanSingh14/"
          target="_blank"
        ...
        >
          ...
          <p className="text-lg md:text-xl font-medium">LeetCode</p>
        </a>

        <hr className="hidden md:block w-px h-20 bg-gray-300" />

        <a
          href="https://github.com/Muskan1415"
          target="_blank"
          rel="noopener noreferrer"
          className="flex flex-col items-center gap-2 hover:scale-110 transition duration-500"
        >
          <FaGithub className="text-5xl text-white" />
          <p className="text-lg md:text-xl font-medium">GitHub</p>
        </a>
      </motion.div>
    </section>
  );
